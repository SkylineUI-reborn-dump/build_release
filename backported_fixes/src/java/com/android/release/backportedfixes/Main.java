 ...
 */
package com.android.release.backportedfixes;

import static java.nio.charset.StandardCharsets.UTF_8;

import com.android.release.backportedfixes.common.ClosableCollection;
import com.android.release.backportedfixes.common.Parser;
...
public final class Main {
    @Parameter(converter = FileConverter.class, required = true)
    List<File> fixFiles;
    @Parameter(names = {"--property_file", "-p"}, converter = FileConverter.class, required = true)
    File propertyFile;
    ...
    public void run() throws Exception {
        try (
                var fixStreams = ClosableCollection.wrap(Parser.getFileInputStreams(fixFiles));
     ...
     static void writeFixesAsAliasBitSet(BackportedFixes fixes, Writer out) {
        ...
        if(printWriter.checkError()){
            throw new RuntimeException("There as an error writing to " +out.toString());
        }
    }
