...
public class CloseableCollectionTest {
    ...
    public void bothClosed() throws Exception {
        ...
        Truth.assertThat(c).comparingElementsUsing(
                Correspondence.transforming(FakeCloseable::isClosed, "is closed")).containsExactly(
                true, true);
    }
    ...
    public void bothFailed() {
        ...
        try {
            ...
            Truth.assertThat(e.getSuppressed()).asList().comparingElementsUsing(
                    Correspondence.transforming(Exception::getMessage,
                            "has a message of ")).containsExactly(
                    "foo close failed", "bar close failed");
        }
    }
}
